...
void generateReport(const struct Student students[], int count);  // FUNKSIONI I RI
...
int main() {
    ...
    do {
        printMenu();
        choice = getValidInt(1, 5, "Enter your choice"); // Tani 5 opsione
        ...
        switch (choice) {
            ...
            case 3:
                ...
                break;
            case 4:
                generateReport(students, studentCount);   // Opsioni i ri për raport
                break;
            case 5:
...
void generateReport(const struct Student students[], int count) {
    ...
    int passingCount = 0;      // Status PASSING
    int needsImprovementCount = 0;
    int atRiskCount = 0;
    
    // Kalojmë nëpër të gjithë studentët me një FOR loop
    for (int i = 0; i < count; i++) {
        ...
        // Përdorim if/else për të klasifikuar statusin
        if (students[i].status == PASSING) {
            passingCount++;
        } else if (students[i].status == NEEDS_IMPROVEMENT) {
            needsImprovementCount++;
        } else if (students[i].status == AT_RISK) {
            atRiskCount++;
        }
    }
    ...
    // Klasifikim shtesë me if/else për mesazhe kuptimplota
    printf("\nPerformance classification:\n");
    if (averageScore >= 80.0) {
        printf("  ★ Excellent overall performance!\n");
    } else if (averageScore >= 60.0) {
        printf("  ★ Good, but there is room for improvement.\n");
    } else if (averageScore >= 40.0) {
        printf("  ★ Needs significant improvement.\n");
    } else {
        printf("  ★ Critical situation. Immediate intervention required.\n");
    }
    
    // Analizë e numrit të studentëve që kalojnë
    if (passingCount >= count * 0.7) {
        printf("  ✓ Majority of students are passing.\n");
    } else if (passingCount > 0) {
        printf("  ⚠ Only %d out of %d students are passing.\n", passingCount, count);
    } else {
        printf("  ✗ No student is passing. Urgent action needed!\n");
    }
